...
const SearcFilter = () => {
  const {
    ...
    yearCar,
    setYearCar,
    cars,IsSearchSection,setIsSearchSection
  } = useContext(WikiCars);
...
if (makerCar && modelCar) {
    const years = cars
    .filter((car) => car.make_display === makerCar && car.model_name === modelCar)
    .map((car) => car.model_year);
  setAvailableYears(Array.from(new Set(years)));
}

   else  if (makerCar) {
      const years = cars
        .filter((car) => car.make_display === makerCar)
        .map((car) => car.model_year);
      setAvailableYears(Array.from(new Set(years))); // Get unique years
    } 
   else  if  (modelCar) {
      const years = cars
        .filter((car) => car.model_name === modelCar)
        .map((car) => car.model_year);
      setAvailableYears(Array.from(new Set(years))); // Get unique years
    } 
    
    else {
      setAvailableYears([]); // Reset if no marca is selected
    }
  ...
  return (
    <form className={IsSearchSection?" flex justify-center absolute bg-white top-[63px]  w-screen py-2 ":"absolute hidden bg-white top-[63px] left-0 w-screen py-2 "}>
      <div className="flex flex-wrap gap-10 justify-center items-center">
        <div className="">

        <select {...register("marca")} id="marca"  className="bg-gray-200 text-gray-700 p-2 rounded-md outline-none focus:ring focus:ring-gray-300 max-w-32 sm:w-auto">
